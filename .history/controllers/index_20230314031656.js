...
document.querySelector("#btnThemNV").onclick = function () {
  ...
  var valid = true;
    ...
    valid = check.checkEmty(nv.account,'error-required-account','Tài Khoản')
    // &&check.checkEmty(nv.yourName,'error-required-name','Tên Nhân Viên')&&check.checkEmty(nv.email,'error-required-email','Email')&&check.checkEmty(nv.password,'error-required-password','Mật Khẩu')&&check.checkEmty(nv.salary,'error-required-luongCB','Lương Cơ Bản')&&check.checkEmty(nv.timework,'error-required-gioLam','Giờ Làm');
    // valid = valid && check.checkLetter(nv.yourName,'error-AllLetter-name','Tên Nhân Viên');
    // valid = valid && check.checkemail(nv.email,'error-email','Email');
    // valid = valid && check.checkLength(nv.account,'error-tknv','Tài Khoản',4,6);
    // valid = valid && check.checkPassword(nv.password,'error-password','Mật Khẩu',6,10);
    // valid = valid && check.checkNumber(nv.salary,'error-luongCB','Lương Cơ Bản',1000000,20000000);
    // valid = valid && check.checkRole(nv.luongChucVu,'error-chucvu');
    // valid = valid && check.checkNumber(nv.timework,'error-gioLam','Giờ Làm',80,200);
...
document.getElementById("btnCapNhat").onclick = function () {
  ...
  // valid = check.checkEmty(nhanVienEdit.account,'error-required-tknv','Tài Khoản')&&check.checkEmty(nhanVienEdit.yourName,'error-required-name','Tên Nhân Viên')&&check.checkEmty(nhanVienEdit.email,'error-required-email','Email')&&check.checkEmty(nhanVienEdit.password,'error-required-password','Mật Khẩu')&&check.checkEmty(nhanVienEdit.salary,'error-required-luongCB','Lương Cơ Bản')&&check.checkEmty(nhanVienEdit.timework,'error-required-gioLam','Giờ Làm');
  // valid = valid && check.checkLetter(nhanVienEdit.yourName,'error-AllLetter-name','Tên Nhân Viên');
  // valid = valid && check.checkemail(nhanVienEdit.email,'error-email','Email');
  // valid = valid && check.checkLength(nhanVienEdit.account,'error-tknv','Tài Khoản',4,6);
  // valid = valid && check.checkPassword(nhanVienEdit.password,'error-password','Mật Khẩu',6,10);
  // valid = valid && check.checkNumber(nhanVienEdit.salary,'error-luongCB','Lương Cơ Bản',1000000,20000000);
  // valid = valid && check.checkRole(nhanVienEdit.position,'error-chucVu');
  // valid = valid && check.checkNumber(nhanVienEdit.timework,'error-gioLam','Giờ Làm',80,200);
  if(!valid){
      return ;
  }
