...
document.querySelector("#btnThemNV").onclick = function () {
  ...
  valid =
    check.checkEmty(nv.tknv, "error-required-tknv", "Tài Khoản") &
    check.checkEmty(nv.name, "error-required-name", "Tên Nhân Viên") &
    check.checkEmty(nv.email, "error-required-email", "Email") &
    check.checkEmty(nv.password, "error-required-password", "Mật Khẩu") &
    check.checkEmty(nv.luongCB, "error-required-luongCB", "Lương Cơ Bản") &
    check.checkEmty(nv.gioLam, "error-required-gioLam", "Giờ Làm");
  valid =
    valid & check.checkLetter(nv.name, "error-AllLetter-name", "Tên Nhân Viên");
  valid = valid & check.checkemail(nv.email, "error-email", "Email");
  valid = valid & check.checkLength(nv.tknv, "error-tknv", "Tài Khoản", 4, 6);
  valid =
    valid &
    check.checkPassword(nv.password, "error-password", "Mật Khẩu", 6, 10);
  valid =
    valid &
    check.checkNumber(
      nv.luongCB,
  ...
  valid =
    valid & check.checkNumber(nv.gioLam, "error-gioLam", "Giờ Làm", 80, 200);
  if (!valid) {
    return;
  }
...
document.getElementById("btnCapNhat").onclick = function () {
  ...
  valid = check.checkEmty(capNhatNV.account,'error-required-tknv','Tài Khoản')&check.checkEmty(capNhatNV.yourName,'error-required-name','Tên Nhân Viên')&check.checkEmty(capNhatNV.email,'error-required-email','Email')&check.checkEmty(capNhatNV.password,'error-required-password','Mật Khẩu')&check.checkEmty(capNhatNV.luongCB,'error-required-luongCB','Lương Cơ Bản')&check.checkEmty(capNhatNV.timework,'error-required-gioLam','Giờ Làm');
  valid = valid & check.checkLetter(capNhatNV.yourName,'error-AllLetter-name','Tên Nhân Viên');
  valid = valid & check.checkemail(capNhatNV.email,'error-email','Email');
  valid = valid & check.checkLength(capNhatNV.account,'error-tknv','Tài Khoản',4,6);
  valid = valid & check.checkPassword(capNhatNV.password,'error-password','Mật Khẩu',6,10);
  valid = valid & check.checkNumber(capNhatNV.salary,'error-luongCB','Lương Cơ Bản',1000000,20000000);
  valid = valid & check.checkRole(capNhatNV.position,'error-chucVu');
  valid = valid & check.checkNumber(capNhatNV.timework,'error-gioLam','Giờ Làm',80,200);
  if(!valid){
      return ;
  }
