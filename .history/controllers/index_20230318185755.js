...
document.querySelector("#btnThemNV").onclick = function () {
  ...
  valid =
    // check.checkRong(nv.account, "tbTKNV", "Tài Khoản") &
    // check.checkRong(nv.yourName, "tbTen", "Tên Nhân Viên") &
    // check.checkRong(nv.email, "tbEmail", "Email") &
    // check.checkRong(nv.password, "tbMatKhau", "Mật Khẩu") &
    // check.checkRong(nv.datawork, "tbNgay"," Ngày làm")
    // check.checkRong(nv.salary, "tbLuongCB", "Lương Cơ Bản") &
    // check.checkRong(nv.timework, "tbGioLam", "Giờ Làm");
  valid = check.chucVu(nv.position, 'tbChucVu')
  valid = check.checkEmail(nv.email, 'error-tbEmail',"Email")
  if (!valid) {
    return;
  }
