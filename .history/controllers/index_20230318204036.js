...
document.getElementById("btnCapNhat").onclick = function () {
  ...
  var valid = true;
  valid = check.checkLength(nhaccount, 'tbTKNV','Tài khoản', 4, 6, "error-tbTKNV")
  valid =check.checkPassword(nhpassword, 'tbMatKhau', 'Password', "error-tbMatKhau")
  valid = check.chucVu(nhposition, 'tbChucVu',"error-tbLuongCB")
  valid = check.checkEmail(nhemail, 'tbEmail',"Email",'error-tbEmail')
  valid = check.checkNumber(nhsalary, 'tbLuongCB',"Lương cơ bản ",1000000, 20000000, "error-tbLuongCB");
  valid = check.checkNumber(nhtimework, 'tbGioLam',"Giờ đã làm ",80, 200, "error-tbGioLam");
  valid = check.chechName(nhyourName, "tbTen","Họ tên","error-tbTen")
  valid = check.checkDate(nhdatawork, 'tbNgay', 'ngày làm', 'error-tbNgay');
  if (!valid) {
    return;
  }
