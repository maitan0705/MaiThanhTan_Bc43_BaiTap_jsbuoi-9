...
document.getElementById("btnCapNhat").onclick = function () {
  ...
  valid = check.checkEmty(nha.account,'error-required-account','Tài Khoản')&check.checkEmty(nha.yourName,'error-required-yourName','Tên Nhân Viên')&check.checkEmty(nha.email,'error-required-email','Email')&check.checkEmty(nha.password,'error-required-password','Mật Khẩu')&check.checkEmty(nha.salary,'error-required-salary','Lương Cơ Bản')&check.checkEmty(nha.timework,'error-required-timework','Giờ Làm');
  valid = valid & check.checkLetter(nha.yourName,'error-AllLetter-yourName','Tên Nhân Viên');
  valid = valid & check.checkemail(nha.email,'error-email','Email');
  valid = valid & check.checkLength(nha.account,'error-account','Tài Khoản',4,6);
  valid = valid & check.checkPassword(nha.password,'error-password','Mật Khẩu',6,10);
  valid = valid & check.checkNumber(nha.salary,'error-salary','Lương Cơ Bản',1000000,20000000);
  valid = valid & check.checkRole(nha.position,'error-chucVu');
  valid = valid & check.checkNumber(nha.timework,'error-timework','Giờ Làm',80,200);
  if(!valid){
      return ;
  }
