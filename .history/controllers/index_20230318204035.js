...
document.getElementById("btnCapNhat").onclick = function () {
  ...
  var valid = true;
  valid = check.checkLength(account, 'tbTKNV','Tài khoản', 4, 6, "error-tbTKNV")
  valid =check.checkPassword(password, 'tbMatKhau', 'Password', "error-tbMatKhau")
  valid = check.chucVu(position, 'tbChucVu',"error-tbLuongCB")
  valid = check.checkEmail(email, 'tbEmail',"Email",'error-tbEmail')
  valid = check.checkNumber(salary, 'tbLuongCB',"Lương cơ bản ",1000000, 20000000, "error-tbLuongCB");
  valid = check.checkNumber(timework, 'tbGioLam',"Giờ đã làm ",80, 200, "error-tbGioLam");
  valid = check.chechName(yourName, "tbTen","Họ tên","error-tbTen")
  valid = check.checkDate(datawork, 'tbNgay', 'ngày làm', 'error-tbNgay');
  if (!valid) {
    return;
  }
