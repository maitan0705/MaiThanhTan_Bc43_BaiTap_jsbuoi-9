...
document.getElementById("btnCapNhat").onclick = function () {
  ...
  valid = check.checkEmty(.account,'error-required-account','Tài Khoản')&check.checkEmty(.yourName,'error-required-yourName','Tên Nhân Viên')&check.checkEmty(.email,'error-required-email','Email')&check.checkEmty(.password,'error-required-password','Mật Khẩu')&check.checkEmty(.salary,'error-required-salary','Lương Cơ Bản')&check.checkEmty(.timework,'error-required-timework','Giờ Làm');
  valid = valid & check.checkLetter(.yourName,'error-AllLetter-yourName','Tên Nhân Viên');
  valid = valid & check.checkemail(.email,'error-email','Email');
  valid = valid & check.checkLength(.account,'error-account','Tài Khoản',4,6);
  valid = valid & check.checkPassword(.password,'error-password','Mật Khẩu',6,10);
  valid = valid & check.checkNumber(.salary,'error-salary','Lương Cơ Bản',1000000,20000000);
  valid = valid & check.checkRole(.position,'error-chucVu');
  valid = valid & check.checkNumber(.timework,'error-timework','Giờ Làm',80,200);
  if(!valid){
      return ;
  }
